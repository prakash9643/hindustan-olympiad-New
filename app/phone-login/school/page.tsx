...
export default function PhoneLogin() {
    ...
    useEffect(() => {
        ...
        if (user && user !== "undefined" && user !== null) {
            if (localStorage.getItem("type") === "team-member" || localStorage.getItem("type") === "school-coordinator") {
                router.push("/team");
            }
        } else {
        }
    ...
    const sendOTP = async (e: any) => {
        ...
        try {
            ...
            if (data.error) {
                error("Invalid mobile number", { duration: 3000, position: "top-right", description: data.error });
            } else {
    ...
    const verifyOTP = async (e: any) => {
        ...
        try {
            ...
            if (data.error) {
                error("Incorrect OTP", { duration: 3000, position: "top-right", description: "Incorrect OTP. Please try again." });
            } else {
                success('OTP verified!', { duration: 3000, position: "top-right", description: "OTP verified successfully." });
                console.log(data);
                console.log(data.user);
                localStorage.setItem("user", JSON.stringify(data.user));
                localStorage.setItem("type", "school-coordinator");
