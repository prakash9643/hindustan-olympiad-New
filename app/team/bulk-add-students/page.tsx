...
export default function BulkAddStudentForm() {
  ...
  const regionKey: string = school.region?.toString() || "";

  // Convert region
  const regionObj = regions.find(r => r.value === regionKey);
  const regionName = regionObj ? regionObj.label : school.region;

  // Convert district ONLY from that region
  const districtList = districts[regionKey as keyof typeof districts] || [];
  const districtObj = districtList.find(
    (d: any) => d.value?.toString() === school.district?.toString()
  );

  const districtName = districtObj ? districtObj.label : school.district;
